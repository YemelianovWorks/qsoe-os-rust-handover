...
Captured: 2026-06-29 11:28 CEST.
...
| `libtaskman` | 23 | 3,013 | Best source of host-testable task-manager modules. `tm_procfs` is Rust-default RC; `tm_cpio`, `tm_cred`, and `tm_sysfs` are Rust opt-in; more candidates are tracked in #143-#153. |
| `lq` | 90 | 17,853 | seL4 task manager, LQ libc wrappers, process, capability, path, memory, syscall, and boot glue. Pure/diagnostic slices only are candidates; LQ pseudo-devices are Rust opt-in. |
...
| Component issues | 10 | Components with concrete Rust artifacts or RC evidence. |
| Backlog, retirement, and inventory issues | 12 | Remaining candidates, deferred areas, retirement gate, and this inventory. |
...
| Task-manager pure or diagnostic modules | #143-#147, #149, #151, #153 | Candidate backlog. Prefer host-tested modules that avoid direct seL4 invocations, spawn, capability ownership, relocation writes, and loader admission. |
...
1. Use #143-#147, #149, #151, and #153 for remaining small task-manager pilots. Prefer
   low-risk pure modules before touching path manager or ELF-adjacent loader
   inputs.
