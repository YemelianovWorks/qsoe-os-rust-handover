...
| `libtaskman` | 19 | 2,311 | Best source of host-testable task-manager modules. `tm_procfs`, `tm_cpio`, `tm_script`, and `tm_elf` are retired to Rust through the shared provider archive; `tm_syscfg` and `tm_sysfs` are Rust-default RCs with C rollback; `tm_cred` and `tm_pathmgr` are Rust opt-in; remaining candidates are tracked in #153. |
...
| **Total** | **810** | **130,255** | QSOE-owned C/asm/linker surface in this checkout, excluding generated build outputs and vendor seL4. |
