...
| `libtaskman` | 23 | 3,013 | Best source of host-testable task-manager modules. `tm_procfs` is Rust-default RC; `tm_cred` and `tm_sysfs` are Rust opt-in; more candidates are tracked in #142-#153. |
...
| Component issues | 9 | Components with concrete Rust artifacts or RC evidence. |
| Backlog, retirement, and inventory issues | 13 | Remaining candidates, deferred areas, retirement gate, and this inventory. |
...
| Task-manager pure or diagnostic modules | #142-#147, #149, #151, #153 | Candidate backlog. Prefer host-tested modules that avoid direct seL4 invocations, spawn, capability ownership, relocation writes, and loader admission. |
...
1. Use #142-#147, #149, #151, and #153 for remaining small task-manager pilots. Prefer
