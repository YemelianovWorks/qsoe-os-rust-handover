...
| `slogger` | Yes: `/sbin/slogger` still uses C normally. | Yes: `QSOE_RUST_SLOGGER=1 make slogger-artifact`; `make rust-slogger-boot-smoke`. | No | No | Behavior spec, C `/dev/slog` readback smoke, Rust ring tests, link smoke, ELF audit, and boot-log comparison. | Extend readback smoke to the Rust-selected service, then Rust-default RC. |
