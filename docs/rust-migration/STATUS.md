...
| `pipe` | Yes: `/sbin/pipe` remains C in normal images. | Yes: `QSOE_RUST_PIPE=1 make pipe-artifact`; `make rust-pipe-link-smoke`; `make rust-pipe-smoke`; `make rust-pipe-data-smoke`. | No | No | C mini-spec, C smoke, `qsoe-pipe` host tests, selector, link smoke, ELF audit, Rust-selected registration boot smoke, and Rust-selected pipe(2) data-path smoke. | Consider a Rust-default RC with C rollback after repeating the data-path smoke on the hosted runner. |
