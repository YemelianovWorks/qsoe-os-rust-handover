...
Last updated: 2026-06-24 08:01 CEST.
...
```text
PR #88: codex/test-msgpass-rs-helper
```
...
2. Decide whether to mark the draft stack ready for review and merge it
   bottom-up from #42 through #88; see #84.
...
4. Use the #86 Rust-selected readback evidence if planning a Rust-default
   `slogger` release candidate, and use #88 if planning a Rust-default
   `test_msgpass` test-image decision; keep C rollback paths available.
5. Continue implementation with an opt-in Rust `pipe` or Rust `tm_procfs`
   provider. Do not start C retirement until the release-candidate gate in
