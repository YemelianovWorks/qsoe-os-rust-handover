...
| FDT parser | `lq/taskman/sys/fdt.c`, `lq/taskman/sys/fdt.h` | Minimal big-endian device-tree scanner for `/chosen`, compatible strings, and properties. Rust-default RC provider exists behind `QSOE_RUST_TM_FDT=1`, with `QSOE_RUST_TM_FDT=0` preserving C rollback; see `TASK_MANAGER_FDT.md` and `TASK_MANAGER_FDT_RC.md`. | Medium: boot config source. |
