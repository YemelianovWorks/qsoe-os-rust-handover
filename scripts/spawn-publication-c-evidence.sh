...
require_pattern "tm_spawn_objcnode_prepare(&objc_plan, &publication"
...

{
    ...
    echo "source=$SRC"
    echo "checked=tm_spawn_publication_plan process record, taskman connection, objcnode handoff, and resume gating with C authority calls"
} > "$OUT"
cat "$OUT"
