#!/usr/bin/env bash
#
# Capture tm_cpio Rust-only retirement evidence.
...
Builds and audits the retired Rust tm_cpio path, verifies that taskman
archives no longer contain C cpio.o, and checks retired selector rejection for
NQ and LQ taskman links.
...
require_retired_selector_rejected() {
    ...
    if "$@" > "$log" 2>&1; then
        fail "$label unexpectedly accepted QSOE_RUST_TM_CPIO=0"
    fi
    grep -Fq 'QSOE_RUST_TM_CPIO must be 1 after C tm_cpio retirement' "$log" ||
        fail "$label rejection did not mention retired tm_cpio selector"
}
...
echo "tm-cpio-evidence.sh: running Rust host model tests"
...
echo "tm-cpio-evidence.sh: verifying NQ Rust-only membership"
"$MAKE" -C "$ROOT/nq/taskman" --no-print-directory \
    QSOE_RUST_TM_CPIO=1 QSOE_RUST_TM_CRED=0 QSOE_RUST_TM_PROCFS=1 \
    QSOE_RUST_TM_SCRIPT=1 QSOE_RUST_TM_SYSFS=0
require_cpio_count nq-rust-retired "$ROOT/nq/build/libtaskman/libtaskman.a" 0
...
echo "tm-cpio-evidence.sh: verifying NQ retired selector rejection"
require_retired_selector_rejected nq \
    "$MAKE" -C "$ROOT/nq/taskman" --no-print-directory QSOE_RUST_TM_CPIO=0

echo "tm-cpio-evidence.sh: verifying LQ Rust-only membership"
"$MAKE" -C "$ROOT/lq" --no-print-directory \
    ...
    taskman
require_cpio_count lq-rust-retired "$ROOT/lq/build/libtaskman/libtaskman.a" 0
...
echo "tm-cpio-evidence.sh: verifying LQ retired selector rejection"
require_retired_selector_rejected lq \
    "$MAKE" -C "$ROOT/lq" --no-print-directory QSOE_RUST_TM_CPIO=0 taskman
